...
public class Race extends JFrame {
	...
	private void prepareToRace() {
		...
		while (input != 's' && input != 'S') {
			...
			case'l': case'L':
				//Lion
				racerList.add(new Lion("Lion", START_LINE, yPos));
				yPos+=RACER_SPACE;
				break;
			case'd': case'D':
				//Dice
				racerList.add(new Dice("Dice", START_LINE, yPos));
				yPos+=RACER_SPACE;
				break;
			default:
				JOptionPane.showMessageDialog( this, "Message" );
	...
	private boolean findWinner() {
		for (Racer r : racerList) {
			if (r.getX() > finishX) {
				//Found winning racer and morph the racer
				r.morph(getGraphics());
				return true;
